...
def makeGenArray(tree, what, ptCut, etaCut, jer,overrideGenArray):
    if overrideGenArray.lower()=="met": 
        return makeMETArray(tree, "met", "Gen",etaCut)
    if overrideGenArray.lower()=="mht": 
        return makeCorrArray(tree, "mht", "Gen",ptCut,etaCut,ROOT.nullptr,jer)
    return makeCorrArray(tree, what, "Gen", ptCut, etaCut, ROOT.nullptr, jer)

    #if False: return makeMETArray(tree, what, "Gen", etaCut)
...
def makeCorrArray(tree, what, obj, ptCorrCut, etaCut, corr, jer={}, _cache={}):
    ...
    if what == "metmht":
        ...
        _cache[_key] = ret
        return ret
    # if doTypeOne or 'metcorr' in what.lower():
    #     # print( 'in type one MET')
    #     # ret = makeMETTypeOneArray(tree, what, obj, etaCut)
    #     ret = makeMETArray(tree, what, obj, etaCut, doTypeOne=doTypeOne,
    #                        ptCorrCut=ptCorrCut, corr=corr, jercalc=jercalc)
    #     _cache[_key] = ret
    #     return ret
    # elif "met" in what:
    if "met" in what.lower():
        # print( 'not in type one MET')
        doTypeOne = "typeone" in what.lower()
...
def makeMETArray(tree, what, obj, etaCut, doTypeOne=False, ptCorrCut=0, corr=ROOT.nullptr, jercalc=None):
    ...
    print 'details:',obj,doTypeOne,what
    if obj == "gen" or (not doTypeOne and not 'corr' in what.lower()):
        print('the usual MET array')
        ret = ROOT.makeMetArray(tree, obj+post);
    else:
        print('the new jetMET array')
        if 'metcorr' in what.lower(): jerCorr = float(what.lower().split('metcorr')[-1])
        else: jerCorr = 0
        if jercalc is None: jercalc=ROOT.JetResolutionCalc()
        ret = ROOT.makeJetMetArray(tree, obj+post, obj, ptCorrCut, etaCut, 
                                   jercalc, corr, doTypeOne, jerCorr);
    progress.done("done, %d entries" % len(ret))
    return ret
...
parser.add_option("--GenArray", dest="overrideGenArray", default="", help="Choose GenArray MET or MHT")
...
if options.var == "ht":
    ...
    what = options.var
elif options.var.startswith("met") and not 'corr' in options.var.lower():
    ...
    qualif = "|#eta| < %.1f" % options.eta
elif 'met' in options.var.lower(): #options.var.startswith("typeOneMet"):
    if options.varlabel is None: options.varlabel = "Corrected E_{T}^{miss}"
    if options.genht    is None: options.genht    = 150
    if options.xmax     is None: options.xmax     = 500
    if options.eff      is None: options.eff      = "0.5,0.9,0.95"
    what = options.var
...
for plotkind in options.plots.split(","):
  ...
  if plotkind != "rate":
    genArray = makeGenArray(signal, what, options.pt, options.eta, jerTool, options.overrideGenArray)
  if plotkind == "isorate":
